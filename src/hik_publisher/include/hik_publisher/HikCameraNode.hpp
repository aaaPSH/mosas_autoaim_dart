...
public:
      ...
      : Node("hik_camera", options)
  {
    ...
    if(tempRet != MV_OK) {
        RCLCPP_WARN(this->get_logger(), "Warning: Failed to set PixelFormat to BayerRG8. Dual stream might fail.");
    }
    ...
    record_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/save_frame", rclcpp::SensorDataQoS());
      ...
      while (rclcpp::ok())
      {
          ...
          if (nRet == MV_OK)
          {
              ...
              switch (OutFrame.stFrameInfo.enPixelType) {
                  case PixelType_Gvsp_BayerRG8: bayer_encoding = sensor_msgs::image_encodings::BAYER_RGGB8; break;
                  case PixelType_Gvsp_BayerGB8: bayer_encoding = sensor_msgs::image_encodings::BAYER_GBRG8; break;
                  case PixelType_Gvsp_BayerGR8: bayer_encoding = sensor_msgs::image_encodings::BAYER_GRBG8; break;
                  case PixelType_Gvsp_BayerBG8: bayer_encoding = sensor_msgs::image_encodings::BAYER_BGGR8; break;
                  default: is_bayer = false; break;
              }
