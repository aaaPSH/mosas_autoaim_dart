...
public:
    GreenDotDetectNode(const rclcpp::NodeOptions & options) : Node("green_dot_detect_node", options)
    {
        ...
        target_pub = this->create_publisher<autoaim_interfaces::msg::GreenDot>("/detections/green_dots",rclcpp::SensorDataQoS().keep_last(1));
...
private:
    ...
    void save_image_to_disk(const cv::Mat &image){
        ...
        std::stringstream ss;
        ss <<  "/home/nvidia/saved_green_dots" << "/DOT_" << std::put_time(std::localtime(&t), "%Y%m%d_%H%M%S")
...
rcl_interfaces::msg::SetParametersResult parametersCallback(
        const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        if (algo_params_changed) {
            ...
            RCLCPP_INFO(this->get_logger(), "[Dynamic] Algorithm Params Updated! V_Low: %d, Area: %.1f", 
    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        // 计算时间差 (秒)
        auto fps_diff = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_fps_time_).count();
        ...
        if (save_images_) {
            auto save_diff_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_save_time_).count();
