...
// 2. 亚像素质心计算
// ==========================================
cv::Point2f GreenDotDetect::computeSubPixelCenter(const cv::Mat &roi_img, const cv::Rect &local_rect) {
    // 确保 ROI 不越界
    cv::Rect safe_rect = local_rect & cv::Rect(0, 0, roi_img.cols, roi_img.rows);
    
    // 如果交集为空，返回几何中心
    if (safe_rect.empty()) {
        return cv::Point2f(local_rect.x + local_rect.width/2.0f, local_rect.y + local_rect.height/2.0f);
    }
...
bool GreenDotDetect::detect(const cv::Mat &raw_image, std::vector<Dot> &dots, bool debug) {
    ...
    if(debug) {
        ...
        cv::circle(debug_vis_full, cv::Point(static_cast<int>(expected_x), static_cast<int>(expected_y)), 1, cv::Scalar(255, 0, 255), -1);
        cv::putText(debug_vis_full, "ROI Search Strip", strip_roi.tl() - cv::Point(0, 5), 
                    cv::FONT_HERSHEY_PLAIN, 1.0, cv::Scalar(255, 0, 255), 1);
    }
    ...
    for(const auto& cnt : contours) {
        ...
        if(area < params.min_area/4.0 || area > params.max_area/4.0) {
            if(debug) {
                ...
                cv::putText(debug_vis_full, cv::format("%.1f", area*4.0), global_pt_draw, cv::FONT_HERSHEY_PLAIN, 0.8, cv::Scalar(0, 255, 255), 1);
            }
            continue;
        }
        ...
        if (maxVal < params.v_low + 10){
            if(debug) {
                cv::rectangle(debug_vis_full, global_r, cv::Scalar(255, 0, 0), 1); // 蓝色
                cv::putText(debug_vis_full, cv::format("%.1f", maxVal), global_pt_draw, cv::FONT_HERSHEY_PLAIN, 0.8, cv::Scalar(255, 0, 0), 1);
            }
            continue;
        }   
        ...
        if (pixel_count > 0) {
            ...
            if (ratio_gr < params.min_gr_ratio || ratio_gb < params.min_gb_ratio) { 
                if(debug) {
                    ...
                    cv::putText(debug_vis_full, cv::format("%.1f,%.1f", ratio_gr, ratio_gb), global_pt_draw, cv::FONT_HERSHEY_PLAIN, 0.7, cv::Scalar(0, 0, 255), 1);
                }
                continue; 
            }
        }
        ...
        if(debug) {
            ...
            cv::Point2f global_center = cv::Point2f(global_r.x, global_r.y) + local_sub_center - cv::Point2f(r.x, r.y);
            cv::drawMarker(debug_vis_full, global_center, cv::Scalar(0,255,0), cv::MARKER_CROSS, 15, 2);
            cv::putText(debug_vis_full, cv::format("%.1f", ratio_gr), global_pt_draw - cv::Point(0, 5), 
            ...
            cv::putText(chip, std::to_string(dots.size()), cv::Point(2, 12), cv::FONT_HERSHEY_PLAIN, 0.8, cv::Scalar(0,255,0), 1);
    ...
    if(debug) {
        ...
        cv::drawMarker(debug_vis_full, cv::Point2f(cx_half, cy_half_val), cv::Scalar(255, 0, 0), cv::MARKER_CROSS, 15, 1);
        ...
        if (magnified_chips.empty()) {
            ...
            cv::putText(vis_chips_strip, "No Targets Detected", cv::Point(10, 35), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(100,100,100), 1);
        } else {
            // 横向拼接
            ...
            cv::putText(vis_chips_strip, "Locked Targets (RGB)", cv::Point(5, 15), cv::FONT_HERSHEY_PLAIN, 1.0, cv::Scalar(0,255,0), 1);
        }
        ...
        if (!mask.empty()) {
            ...
            // 在图上写字提示
            cv::putText(mask_display, "ROI Binary Mask (x3)", cv::Point(5, 25), cv::FONT_HERSHEY_PLAIN, 1.5, cv::Scalar(128), 2);
